# setup Hadoop Bin path:
    ...
    For local IntelliJ Spark practice (using SparkSession.builder().master("local[*]")), you do not need
    to set up Hadoop or its bin/ path at all.
...
# When running Spark on a newer JDK (17/21).
    Spark (or one of its deps) is still trying to use internal sun.* classes, with the Java module system
    those internals are blocked unless you explicitly open/export them.
